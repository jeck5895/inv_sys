      ...
      <modal>
        <template v-slot:modal-title>
          <h5 class="text-white">
            ...
            <span v-if="form_type === 'EDIT' && category.name">
        ...
        <template v-slot:modal-body>
          ...
          <Form :item="category" :loading="submitting" @on-submit="onSubmit" />
        </template>
      </modal>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    category() {
      const item = this.$store.getters["CATEGORIES/GET_CATEGORY"];
  ...
  methods: {
    ...
    handleSearch(keyword) {
      const url = `/api/categories?q=${keyword}&page=${this.current_page}&per_page=${this.page_size}&order_by=${this.order_by}&sort_by=${this.sort_by}`;
      this.fetchItems(url);
    },
    toPage(page) {
      const url = `/api/categories?q=${this.keyword}&page=${page}&per_page=${this.page_size}&order_by=${this.order_by}&sort_by=${this.sort_by}`;
      this.fetchItems(url);
    },
      ...
      this.$dialog
        ...
        .then(dialog => {
          this.delete(item.id).then(() => {
            dialog.close();
            this.fetchItems(
              `/api/categories?q=${this.keyword}&page=${this.current_page}&per_page=${this.page_size}&order_by=${this.order_by}&sort_by=${this.sort_by}`
            );
          });
        })
        .catch(() => {});
