      ...
      <modal>
        <template v-slot:modal-title>
          <h5 class="text-white">
            ...
            <span v-if="form_type === 'EDIT' && model.name">
        ...
        <template v-slot:modal-body>
          ...
          <Form :item="model" :loading="submitting" @on-submit="onSubmit" />
        </template>
      </modal>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    model() {
      const item = this.$store.getters["UNITS/GET_UNIT"];
  ...
  methods: {
    ...
    handleSearch(keyword) {
      const url = `/api/models?q=${keyword}&page=${this.current_page}&per_page=${this.page_size}&order_by=${this.order_by}&sort_by=${this.sort_by}`;
      this.fetchItems(url);
    },
    toPage(page) {
      const url = `/api/models?q=${this.keyword}&page=${page}&per_page=${this.page_size}&order_by=${this.order_by}&sort_by=${this.sort_by}`;
      this.fetchItems(url);
    },
      ...
      this.$dialog
        ...
        .then(dialog => {
          this.delete(item.id).then(() => {
            dialog.close();
            this.fetchItems(
              `/api/models?q=${this.keyword}&page=${this.current_page}&per_page=${this.page_size}&order_by=${this.order_by}&sort_by=${this.sort_by}`
            );
          });
        })
        .catch(() => {});
